...
### 8. 多仓与共享规范要求
1. 多仓项目必须明确哪些规范是组织级共享，哪些文档是项目级事实。
2. 共享规范目录、共享脚本、共享部署模板必须有稳定来源，不得每个仓库各自复制后长期漂移。

### 9. 文档与代码边界
1. 设计稿、SQL、部署文件、运行脚本、正式文档必须有独立目录，不得长期混入源码目录根部。
2. 临时排障脚本、一次性数据修复脚本必须有归档策略，不得无限堆积在主目录。

### 10. 评审附加清单
1. 目录结构是否便于新人和 AI 快速定位。
2. 是否存在语义不明、重复、历史堆积严重的目录。
3. 多项目仓库中是否能清楚区分共享资产与项目私有资产。
