...
## 14. 企业级深化要求
1. 后端架构必须显式区分协议适配、用例编排、领域规则、基础设施集成四类职责，不得把请求处理、事务控制、规则判断、SQL 拼装混在同一层。
2. 控制层只负责协议转换、参数接收、权限入口、响应组装和错误映射，不承担业务流程编排。
3. Service / Application 层负责用例编排、事务边界、幂等控制、审计触发和跨资源协调。
4. Domain 层负责稳定业务规则、状态机、策略判断和领域对象行为；不得依赖 Web 框架细节。
5. Infrastructure 层负责数据库、缓存、搜索、消息、外部系统适配，不得把业务决策埋在 Mapper 或客户端调用里。

## 15. 事务与边界要求
1. 事务边界必须收敛在应用服务层，不得在 Controller、Repository 或外部回调适配层随意开启。
2. 写路径必须同时考虑幂等、审计、权限、日志和失败补偿，而不是只保证“写成功”。
3. 跨数据库、跨缓存、跨消息或跨外部系统的流程，必须明确一致性策略和补偿策略。
4. 复杂审批流、计费流、任务流应建模为显式状态迁移或策略对象，而不是长条件分支。

## 16. Review 附加清单
1. 控制层是否过厚，是否混入业务规则、事务或数据访问。
2. Service 是否只是在转调 Repository，而没有形成清晰用例边界。
3. Repository / Mapper 是否夹带业务判断、权限过滤或租户规则。
4. 事务、幂等、审计、异常、日志是否有统一落点，而不是每个接口自由发挥。
