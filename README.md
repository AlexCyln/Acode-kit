...
阶段驱动执行   → 7 阶段严格顺序（需求细化 → UI设计 → 数据API → 脚手架 → TDD实现 → 测试 → 部署）
```

每个 Gate 要求用户明确确认后才能继续，不允许 AI 自行跳过。Pencil 设计工具仅在阶段 2（UI/页面结构化）才允许使用。

### 工作流规则

**前端页面**（Pencil + shadcn 可用时，仅在阶段 2 使用）：
1. Pencil 设计稿 → 用户确认
2. shadcn 组件库构建
3. 按设计稿 1:1 还原（阶段 5）
...
4. **Gate 4**：创建项目目录结构 + 安装依赖 + 填充项目文档 → 等你确认
5. 全部门控通过后，进入 7 阶段严格顺序执行（需求细化 → UI设计 → 数据API → 脚手架 → TDD实现 → 测试 → 部署）
...
Stage-driven execution → 7 stages in strict order (requirements → UI design → data/API → scaffold → TDD impl → test → deploy)
```

Each gate requires explicit user confirmation before proceeding. The AI cannot skip gates. Pencil design tools are only allowed at stage 2 (UI/page structuring).
...
4. **Gate 4**: Create project directory, install dependencies, populate project docs → waits for your confirmation
5. After all gates pass, enters 7-stage execution (requirements → UI design → data/API → scaffold → TDD → test → deploy)
